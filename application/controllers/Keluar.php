...
class Keluar extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['keluar'] = $this->db->query("SELECT * FROM tbl_keluar RIGHT JOIN tbl_masuk ON tbl_keluar.plat_masuk= tbl_masuk.plat_masuk WHERE tgl_jam_keluar LIKE '" . date('Y-m-d') . "%' AND status_keluar = '1'")->result_array();
		// die(print_r($data));
		$this->load->view('parkirkeluar', $data, FALSE);
	}
	public function kendaraankeluar($value = '')
	{
		$kode = str_replace("'", "", $this->input->post('plat'));
		...
		$sqlcek = $this->db->query("SELECT * FROM tbl_masuk RIGHT JOIN tbl_kendaraan ON tbl_masuk.kd_kendaraan = tbl_kendaraan.kd_kendaraan WHERE plat_masuk = '" . $kode . "' AND status_masuk = '1' ")->row_array();
		if ($sqlcek['plat_masuk']) {
			// echo "ss";
			// die();
